...
You can also run VirSorter locally [using Docker](https://github.com/simroux/VirSorter), or by manually installing the codebase. A local installation requires you to download additional data files (4.2 GB downloaded, 13 GB when uncompressed) that VirSorter needs to run. This data pack was originally made available on CyVerse (see instructions [here](https://github.com/simroux/VirSorter)) and was only compatible with HMMER version 3.0.  This data pack has been converted to support use with HMMER version 3.1b2 and is available [here](https://doi.org/10.5281/zenodo.1168727 "Data Pack from VirSorter: mining viral signal from microbial genomic data") which will be downloaded later using a simple `wget` command. 

#### Local installation instructions
To manually install the VirSorter codebase locally (recommended), do the following. I have only tested it on Linux (Ubuntu and CentOS).
...
make
```
- I have forked the original VirSorter repository (available at https://github.com/brymerr921/VirSorter.git). This version runs faster as several steps are now parallelized. These changes are awaiting a pull request.
...
The original HMMs made available by Roux et al (2015) on CyVerse were formatted for HMMER version 3.0. The HMMs in `virsorter-data-v2.tar.gz` that you just downloaded are built for HMMER version 3.1b2. The following command was used to convert the original data pack (downloaded from CyVerse) so it is now formatted for HMMER version 3.1b2.
```
for i in */*.hmm; do echo "Converting ${i}..."; hmmconvert ${i} > ${i}.new; mv ${i}.new ${i}; hmmpress -f ${i}; done
```
To run VirSorter, you must point it to the location of the extracted `virsorter-data` datapack by passing "--data-dir /path/to/virsorter-data" on the command line each time you run VirSorter.
